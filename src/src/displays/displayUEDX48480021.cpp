/************************************************************************************************
   Файл для дисплея UEDX48480021-MD80ET (480x480) ST7701S RGB Panel
   Based on ST7701 implementation
   Adapted for UEDX48480021-MD80ET with ST7701S Type4 preset
************************************************************************************************/
...
// Объявление init-последовательности ST7701 Type4 из Arduino_GFX (BGR mode для UEDX48480021)
...
// Пост-команды ST7701 включены всегда (обязательны для работы панели)
...
void DspCore::initDisplay() {
  ...
  // 0. Инициализация RST пина согласно гайду UEDX48480021
  pinMode(ST7701_RST, OUTPUT);
  digitalWrite(ST7701_RST, HIGH);  // RST в HIGH (неактивное состояние)
  delay(10);
  digitalWrite(ST7701_RST, LOW);   // RST в LOW (сброс)
  delay(10);
  digitalWrite(ST7701_RST, HIGH);  // RST в HIGH (рабочее состояние)
  delay(50);  // Задержка после сброса
  ...
  if (!bus) {
    ...
    // Инициализация шины согласно гайду UEDX48480021
    if (!bus->begin()) {
  ...
  // 2. Создание объекта RGB-панели с указанием всех пинов и таймингов UEDX48480021
  if (!rgbpanel) {
    ...
    rgbpanel = new Arduino_ESP32RGBPanel(
        ...
        // Пины КРАСНОГО канала (R0-R4) - SWAPPED с синими согласно UEDX48480021 гайду
        ...
        // Пины СИНЕГО канала (B0-B4) - SWAPPED с красными согласно UEDX48480021 гайду
        ST7701_B0, ST7701_B1, ST7701_B2, ST7701_B3, ST7701_B4,
        // Параметры таймингов (горизонтальные) - CRITICAL для UEDX48480021: hfp=50, hsync=8, hbp=10
        /* hsync_polarity */ 1, /* hsync_front_porch */ 50, /* hsync_pulse_width */ 8, /* hsync_back_porch */ 10,
        // Параметры таймингов (вертикальные) - CRITICAL для UEDX48480021: vfp=8, vsync=2, vbp=18
        /* vsync_polarity */ 1, /* vsync_front_porch */ 8, /* vsync_pulse_width */ 2, /* vsync_back_porch */ 18,
        // Параметры тактирования и формата данных - CRITICAL для UEDX48480021: 8MHz, pclk_active_neg=0
  ...
  if (!output_display) {
    ...
    // CRITICAL: Используем ST7701 Type4 init-последовательность (BGR mode) для UEDX48480021
  ...
  if (!gfx) {
    ...
    // Пост-команды ST7701S для установки формата пикселя, инверсии и MADCTL
    if (bus) {
      // Формат пикселя: 18-bit (0x66)
      ...
      // Возврат на страницу 0x00 и установка MADCTL = 0x00 (RGB порядок)
  ...
  // 5. Включение подсветки - CRITICAL: Active LOW для UEDX48480021
  pinMode(ST7701_BL, OUTPUT);
  digitalWrite(ST7701_BL, LOW);  // LOW = включено для UEDX48480021
  delay(50); // Задержка для стабилизации подсветки
...
{ 
    ...
    // Уменьшаем задержку
    delay(100);
  // Принудительно обновляем экран после отрисовки логотипа
  if (gfx) gfx->flush();
}
...
void DspCore::sleep(void) { 
  ...
  TAKE_MUTEX();
  displayOff();
  // Выключаем подсветку RGB панели - Active LOW: HIGH = выключено
...
void DspCore::wake(void) {
  ...
  TAKE_MUTEX();
  displayOn();
  // Включаем подсветку RGB панели - Active LOW: используем PWM для начальной яркости
  #if defined(ENABLE_BRIGHTNESS_CONTROL)
    analogWrite(ST7701_BL, map(config.store.brightness, 0, 100, 255, 0));
  #else
    analogWrite(ST7701_BL, 0);  // Максимальная яркость для Active LOW
  #endif
  GIVE_MUTEX();
}
...
void DspCore::setBrightness(uint8_t brightness) {
  TAKE_MUTEX();
  // UEDX48480021: подсветка Active LOW - используем PWM для плавной регулировки
  // При Active LOW: 0 = максимальная яркость, 255 = минимальная яркость
  if (brightness == 0) {
    digitalWrite(ST7701_BL, HIGH);  // Полностью выключено
  } else {
    // Инвертируем значение для Active LOW: 100% -> 0, 0% -> 255
