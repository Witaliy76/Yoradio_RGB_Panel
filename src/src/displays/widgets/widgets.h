...
class TextWidget: public Widget {
  ...
  protected:
    struct TextCache {
        ...
        char* value;
        TextCache* next;
        TextCache(const char* k, char* v) : key(k), value(v), next(nullptr) {}
    };
  ...
  protected:
    void _draw();
    uint16_t _realLeft();
    char* _getCachedText(const char* txt);
    void _addToCache(const char* txt, char* value);
    void _clearCache();
};
...
class FillWidget: public Widget {
  ...
  protected:
    ...
    void _draw();
    void _clear();
    void _fastFillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color);
};
