...
/***************************************************************/
  ...
  setCursor(_timeleft+_dotsLeft +1, clockTop-CHARHEIGHT+75);
  print(":");                                     /* print dots */
  setFont();
...
void DspCore::_clockDate(){
  if(_olddateleft>0)
  dsp.fillRect(_olddateleft,  clockTop+64, _olddatewidth, CHARHEIGHT*2, config.theme.background);	//очистка надписи даты
...
//  setFont(NULL); // Folosește fontul implicit
  ...
  setCursor(_timeleft, clockTop +63);
  print(_timeBuf);
  setFont();
...
void DspCore::wake(void) {
  ...
  TAKE_MUTEX();
  displayOn();
  digitalWrite(GFX_BL, HIGH); // включить подсветку при пробуждении
  GIVE_MUTEX();
}
