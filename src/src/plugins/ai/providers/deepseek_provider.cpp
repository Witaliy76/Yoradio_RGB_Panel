/**
 * deepseek_provider.cpp - Реализация провайдера DeepSeek
 * Описание: Реализация LLMProvider для DeepSeek API, построение промптов, HTTP запросы, парсинг JSON
 * Автор: W76W, 4pda.to
 * Дата: 21.12.2025
...
String DeepSeekProvider::_buildPrompt(
    ...
    "      Примеры: прорывной хит, знаковая история записи, необычный факт о записи.\n"
    ...
    "   - дата/год релиза (только как запасной вариант)\n\n"
    "   ФИЛЬМЫ/СЕРИАЛЫ (очень рискованно):\n"
    "   - По умолчанию ЗАПРЕЩЕНО утверждать, что трек входит в саундтрек фильма/сериала.\n"
    "   - Разрешено ТОЛЬКО если это сверхобщеизвестный хрестоматийный факт и ты уверен на 0.99.\n"
    "   - Если уверенность ниже 0.99 — НЕ пиши про фильм, выбери mode=\"listen\" или ok=false.\n"
    "   - Любые формулировки \"входит в официальный саундтрек\", \"использовался в фильме\" без 0.99 — запрещены.\n\n"
    ...
    "- mode=\"fact\" НЕ должен содержать описания звучания. Fact — только проверяемые сведения вне музыки (релиз/хит/награда).\n"
