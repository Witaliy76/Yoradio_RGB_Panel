...
const float MIN_VU_THRESHOLD = 5.0f; // минимальный порог
const float VU_GAIN = 1.0f;          // коэффициент усиления
const float VU_FLOOR = 5.0f;         // нижний порог
//****************************************************************************************
 uint16_t Audio::get_VUlevel(uint8_t numBands){
  uint8_t L = map(vuLeft, 0,200, 0, numBands);
  uint8_t R = map(vuRight, 0, 200, 0, numBands);
  if (L > numBands) L = numBands;
  if (R > numBands) R = numBands;
  return (L << 8) | R;
}
//****************************************************************************************
